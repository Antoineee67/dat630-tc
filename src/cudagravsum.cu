...
#define pos_mass_vector MAKE_TYPE(float, PMDIM)
#else
#define cuda_vector MAKE_TYPE(double, NDIM)
#define pos_mass_vector MAKE_TYPE(double, PMDIM)
#endif
...
static std::vector<pos_mass_vector> h_pos_mass_vecs;
...
__global__ void cuda_node_calc_coalescing_kernel(real eps2, pos_mass_vector* pos_mass_lists, uint32_t* offset, size_t* bodies_to_process,
    real *out_phi_list, cuda_vector* out_acc_list, size_t n_bodies){
    ...
    size_t current_body_index = bodies_to_process[list_index];
    pos_mass_vector current_body_pos_mass = pos_mass_lists[start-1];
    //real current_body_mass = current_body_pos_mass.w;
    cuda_vector current_body_pos = make_float3(current_body_pos_mass.x, current_body_pos_mass.y, current_body_pos_mass.z);

    for (uint32_t i = start; i < end; i++)
    {   
        //size_t loop_body_index = interact_lists[i];
        pos_mass_vector loop_body_pos_mass = pos_mass_lists[i];
        real loop_body_mass = loop_body_pos_mass.w;
        cuda_vector loop_body_pos = make_float3(loop_body_pos_mass.x, loop_body_pos_mass.y, loop_body_pos_mass.z);

        dr = loop_body_pos - current_body_pos;
        dr2 = dot(dr, dr);
        dr2 += eps2;
        drab = sqrt(dr2);
        phi_p = loop_body_mass/drab;
        local_phi0 -= phi_p;
        mr3i = phi_p/dr2;
        //ADDMULVS(local_acc0, dr, mr3i);
        local_acc0 += dr * mr3i;
...
void cuda_gravsum(bodyptr current_body, cell_ll_entry_t *cell_list_tail, cell_ll_entry_t *body_list_tail) {
    ...
    h_pos_mass_vecs.push_back(make_float4(VECTOR_TO_CUDA_VECTOR(Pos(current_body)), Mass(current_body)));
    ...
    while (curr_list_entry->priv != nullptr) {       
        ...
        cellptr loop_cell_index = celltab + curr_list_entry->index;
        h_pos_mass_vecs.push_back(make_float4(VECTOR_TO_CUDA_VECTOR(Pos(loop_cell_index)), Mass(loop_cell_index)));
    ...
    while (curr_list_entry->priv != nullptr) {     
        ...
        bodyptr loop_body_index = bodytab + curr_list_entry->index;
        h_pos_mass_vecs.push_back(make_float4(VECTOR_TO_CUDA_VECTOR(Pos(loop_body_index)), Mass(loop_body_index)));
        curr_list_entry = curr_list_entry->priv;
    } 

    h_offset.push_back(h_pos_mass_vecs.size()); // = end
    h_bodies_to_process.push_back((size_t)(current_body-bodytab));

    if (h_bodies_to_process.size()==BLOCK_SIZE*16){
        cuda_gravsum_dispatch();
        //h_interact_vecs.clear();
        h_pos_mass_vecs.clear();
...
void cuda_gravsum_dispatch()
{
    ...
    pos_mass_vector* h_pos_mass_vecs_raw = h_pos_mass_vecs.data();
    ...
    pos_mass_vector* d_pos_mass_vecs_raw;
    cudaMalloc(&d_pos_mass_vecs_raw, sizeof(pos_mass_vector)*h_pos_mass_vecs.size());
    ...
    cudaMemcpyAsync(d_bodies_to_process_raw, h_bodies_to_process_raw, sizeof(size_t)*nBodiesToProcess, 
                    cudaMemcpyHostToDevice, localCudaStreams[idle_stream]);
    cudaMemcpyAsync(d_pos_mass_vecs_raw, h_pos_mass_vecs_raw, sizeof(pos_mass_vector)*h_pos_mass_vecs.size(),
                    cudaMemcpyHostToDevice, localCudaStreams[idle_stream]);
    ...
    cuda_node_calc_coalescing_kernel<<<nrGrids, blocksize, 0, localCudaStreams[idle_stream]>>>(eps2, 
        d_pos_mass_vecs_raw, // 1D vector which links all interact lists
        d_offset_raw, // offset for each body in 1D vector
        d_bodies_to_process_raw, 
        d_out_phi_raw, // phi
        d_out_acc_raw, // acc
    ...
    cudaFreeAsync(d_pos_mass_vecs_raw, localCudaStreams[idle_stream]);
